...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["name"]) xor (empty($_POST["website"]))) {
            echo "<span class=\"error\"> Error: First Name and Website Required </span>";
        ...
        }else if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~|]/i",
            $_POST["website"])) {
            echo "<span class=\"error\"> Error: Website is in wrong format (WWW.MYWEBSITE.COM)</span>";
        }else {
        $name = val($_POST["name"]);
